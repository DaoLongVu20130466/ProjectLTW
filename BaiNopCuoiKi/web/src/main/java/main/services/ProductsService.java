...
//                    "WHERE food.ID_SIZE=\"SIZE1\"\n "
            ResultSet rs = stmt.executeQuery("select FOOD.ID_FOOD , FOOD.FOOD_NAME, FOOD.ID_TYPE, PRICE.PRICE , IMAGE.IMG_0\n" +
...
//        }
    ...
    public List<Products> getAllproductBySale() {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("select FOOD.ID_FOOD , FOOD.FOOD_NAME, FOOD.ID_TYPE, PRICE.PRICE , IMAGE.IMG_0\n" +
                    "                    from (food join price on food.ID_PRICE = price.ID_PRICE) join image on food.ID_IMG = image.ID_IMG \n" +
                    "                    WHERE food.ID_SIZE= \"SIZE1\" AND food.ID_SALE = \"SALE1\""
            );
            while (rs.next()) {
...
//
        ...
        ArrayList<Products> allProduct = (ArrayList<Products>) getInstance().getAllProductByPage(1);
        for (Products t : allProduct
             ) {
            System.out.println(t.getID_food());
        }
    }
}
